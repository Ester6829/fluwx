...
class WeChatAuthByQRCodeFinishedResponse extends WeChatResponse {
  WeChatAuthByQRCodeFinishedResponse.fromMap(Map map)
      : authCode = map['authCode'],
        qrCodeErrorCode =
            (_authByQRCodeErrorCodes[map[_errCode]] ?? AuthByQRCodeErrorCode.unknown),
        super._(map[_errCode], map[_errStr]);
...
class FluwxOpenBusinessViewResponse extends WeChatResponse {
  final FluwxBusinessScene businessType;
  final String? result;

  FluwxOpenBusinessViewResponse({
    required int? errCode,
    required String? errStr,
    required this.businessType,
    this.result,
  }) : super._(errCode, errStr);

  factory FluwxOpenBusinessViewResponse.fromJson(Map<String, dynamic> json) {
    final businessType = json['businessType'];
    final extMsg = json['extMsg'];
    String? result;
    if (extMsg is String && extMsg.isNotEmpty) {
      try {
        final extMsgJson = jsonDecode(extMsg);
        result = extMsgJson['result'];
      } catch (e) {
        // Handle JSON parsing error if necessary
      }
    }

    return FluwxOpenBusinessViewResponse(
      errCode: json['errCode'] ?? 0,
      errStr: json['errStr'] ?? '',
      businessType: FluwxBusinessScene.values.firstWhere(
        (element) => element.toString().split('.').last == businessType,
        orElse: () => FluwxBusinessScene.requestMerchantTransfer,
      ),
      result: result,
    );
  }
